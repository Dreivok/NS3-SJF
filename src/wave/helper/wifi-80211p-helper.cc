...
Wifi80211pHelper::Install (const WifiPhyHelper &phyHelper, const WifiMacHelper &macHelper, NodeContainer c) const
{
  ...
  try
    {
      const QosWifipMacHelper& qosMac = dynamic_cast<const QosWifipMacHelper&> (macHelper);
  ...
  try
    {
      const NqosWifipMacHelper& nqosMac = dynamic_cast<const NqosWifipMacHelper&> (macHelper);
      isWaveMacHelper = true;
      if (&nqosMac == 0)
        {
          NS_FATAL_ERROR ("it could never get here");
  ...
  if (!isWaveMacHelper)
    {
      NS_FATAL_ERROR ("the macHelper should be either QosWifipMacHelper or NqosWifipMacHelper"
                      ", or should be the subclass of QosWifipMacHelper or NqosWifipMacHelper");
    }
