...
OcbWifiMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::OcbWifiMac")
    ...
    .AddAttribute ("MacExtensionSupport",
                   "This Boolean attribute is set to enable 802.11e/WMM-style QoS support at this STA",
                   BooleanValue (false),
                   MakeBooleanAccessor (&OcbWifiMac::SetMacExtensionSupported,
                                        &OcbWifiMac::GetMacExtensionSupported),
                   MakeBooleanChecker ())
  ;
  return tid;
}
...
void
OcbWifiMac::SetMacExtensionSupported (bool enable)
{
  m_macExtensionSupport = enable;
  if (enable)
  {
	  CreateWaveMacLow ();
	  CreateWaveEdcaQueue (AC_VO);
	  CreateWaveEdcaQueue (AC_VI);
	  CreateWaveEdcaQueue (AC_BE);
	  CreateWaveEdcaQueue (AC_BK);
  }
}

bool
OcbWifiMac::GetMacExtensionSupported (void) const
{
  return m_macExtensionSupport;
}
...
void
OcbWifiMac::SwitchQueueToChannel (uint32_t channelNumber)
{
  NS_LOG_FUNCTION (this << channelNumber);
  for (EdcaQueues::iterator i = m_edca.begin (); i != m_edca.end (); ++i)
    {
      Ptr<WaveEdcaTxopN> edca = DynamicCast<WaveEdcaTxopN> (i->second);
      edca->SwitchToChannel (channelNumber);
    }
}
void
OcbWifiMac::QueueStartAccess (void)
{
  NS_LOG_FUNCTION (this);
  for (EdcaQueues::iterator i = m_edca.begin (); i != m_edca.end (); ++i)
    {
...
void
OcbWifiMac::CreateWaveEdcaQueue (enum AcIndex ac)
{
//  NS_ASSERT (m_qosSupported == true);
  ...
  Ptr<WaveEdcaTxopN> edca = CreateObject<WaveEdcaTxopN> ();
  edca->SetLow (m_low);
...
void
OcbWifiMac::SetWaveDevice (Ptr<NetDevice> device)
{
  DynamicCast<WaveMacLow>(m_low)->SetWaveDevice (device);
}
